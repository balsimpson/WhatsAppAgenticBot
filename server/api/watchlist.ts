...
import {
  ...
  export default defineEventHandler(async (event) => {
	...
	if (!txt || !assistant_id) {
	  throw createError({
		statusCode: 400,
		statusMessage: "Missing required parameters: txt or assistant_id",
	  });
	}
	...
	try {
	  ...
	  const newContent = `${fileContent}\n${txt}`.trim();
	  const newFile = await createAndUploadFile(newContent);
  
	  // @ts-ignore
	  const vector_store_id = await getVectorStoreId(assistant_id);
	  if (!vector_store_id) {
		throw new Error("No vector store found for the assistant.");
	  }
  
	  await Promise.all([
		// @ts-ignore
		file_id ? deleteFile(file_id, vector_store_id).catch(console.error) : Promise.resolve(),
		// @ts-ignore
		addFileToVectorStore(newFile.id, vector_store_id),
		// @ts-ignore
		updateStorage(kvstorage, newFile.id, newContent),
		// @ts-ignore
		logWatchlistUpdate(txt),
	]);
	...
	await addLogEntry({
		type: "error",
		// @ts-ignore
		content: `Watchlist update failed: ${error.message}`,
	});
	throw createError({
		statusCode: 500,
		// @ts-ignore
...
async function createAndUploadFile(content: string) {
...
async function getVectorStoreId(assistant_id: string) {
  ...
  async function updateStorage(kvstorage: any, newFileId: string, content: string) {
	await Promise.all([
	  kvstorage.setItem("file_id", newFileId),
	  kvstorage.setItem("watchlist", content),
	]);
  }
