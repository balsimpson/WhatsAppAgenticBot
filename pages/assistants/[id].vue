	...
	<div class="w-full max-w-2xl p-3 mx-auto mt-12">
		...
		<div v-else-if="assistant">
			<div class="flex justify-between">
				...
				<div class="text-sm text-gray-400">{{ assistant.model }}</div>
			</div>
			<div class="text-sm text-gray-400">{{ assistant.id }}</div>
			...
			>
				<div class="flex items-center gap-x-4">
					...
					<div
						...
						class="inline-flex items-center px-2 border rounded"
					>
						<svg
						...
						<span class="pl-1 text-xs font-semibold">{{ functionCount }}</span>
					</div>
				</div>

				<div class="mt-2 text-sm font-semibold">
					{{ assistant.created_at ? formatDate(assistant.created_at) : "" }}
				</div>
			</div>
...
<script setup>
	...
	const functionCount = computed(() => {
		if (!assistant.value.tools) return 0;
		return assistant.value.tools.filter((tool) => tool.type === "function")
			.length;
	});
	...
	function getFunctionCode(func) {
		return func.code || "Function code not available";
	}

	const formattedTools = computed(() => {
		// console.log(assistant.value.tools)
		if (!assistant.value.tools) return "";
		return assistant.value.tools
			.filter((tool) => tool.type !== "function")
			.map((tool) => formatToolName(tool.type))
			.join(", ");
	});
